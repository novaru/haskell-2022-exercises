...
-- DON'T FORGET TO SPECIFY THE TYPE IN HERE
lastDigit :: Int -> Int
lastDigit n = if n < 0 then mod (-n) 10 else n `mod` 10
...
lowerAndGreater n list = show n ++ " is greater than " ++ show lo ++ " elements and lower than " ++ show hi ++ " elements"
  where
    hi = (length . filter (> n)) list
    lo = (length . filter (< n)) list
